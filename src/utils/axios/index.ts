...
import type { AxiosTransform, CreateAxiosOptions, RequestOptions, Result } from '#/utils';
  ...
  transformResponseHook: (res: AxiosResponse<Result>, options: RequestOptions) => {
    ...
    if (hasSuccess) {
      ...
      if (isNull(successMsg) || isUnDef(successMsg) || isEmpty(successMsg)) {
        successMsg = t(`status.operationSuccess`);
      }

      if (options.successMessageMode === RequestMessageModeEnum.MODAL) {
        createSuccessModal({ title: t('tip.successTip'), content: successMsg });
      } else if (options.successMessageMode === RequestMessageModeEnum.MESSAGE) {
    ...
    if (options.errorMessageMode === RequestMessageModeEnum.MODAL) {
      createErrorModal({ title: t('tip.errorTip'), content: timeoutMsg });
    } else if (options.errorMessageMode === RequestMessageModeEnum.MESSAGE) {
    ...
    deepMerge(
      {
        ...
        requestOptions: {
          ...
          formatDate: true,
          // 消息提示类型
          errorMessageMode: RequestMessageModeEnum.MESSAGE,
          // 接口地址
